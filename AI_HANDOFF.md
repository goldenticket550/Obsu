...
- **M5 onward:** business-engine calc services + tests, live dashboard numbers, Ask OBSIDIAN (Claude API + tools), natural-language trip entry, customer intelligence, field test. None started.
...
Do the smallest runnable slice, verify it runs, commit, update `CURRENT_STATE.md` and this file, then proceed. One sub-phase at a time (build rule #1/#4). **Do not start M5 until the owner confirms M4 and says go.** See `docs/ROADMAP.md` (RIDES MVP track) and `RETURN_CHECKLIST.md`.
