...
- **Current sub-phase:** **M7 — Ask OBSIDIAN. ✅ BUILT & statically verified (tsc + M5 tests + next build + boot + client-bundle leak check, 2026-07-20); owner to test with an ANTHROPIC_API_KEY set.**
- **Next sub-phase:** **M8 — Natural-language trip entry. NOT STARTED. Do not begin until the owner explicitly says go.**
...
- **M8 onward:** natural-language trip entry, customer intelligence, field test. None started.
...
M7 is built (Ask OBSIDIAN, Claude tool-use over the M5 calcs); owner to test with a real ANTHROPIC_API_KEY. `getInactiveCustomers` was intentionally deferred to M9 (customer intelligence), where inactive-customer detection lives.

**M8 (Natural-language trip entry):**

> Parse free text (e.g. "log a $320 ride from Brooklyn to JFK for Ashley, $40 gas, $15 tolls") into a **structured trip proposal**, show it to the owner to confirm / edit / cancel, and **save only on confirm** — reusing the M4 create path (find-or-create customer + inline gas/tolls/other expenses, money in cents). Claude proposes; nothing is written without explicit confirmation (Level 2 → Level 3). Likely a Claude call that returns a structured proposal (tool/`output_config.format`) rendered into the M4 trip form for review. Keep to this sub-phase only; stop and verify before M9.

Do the smallest runnable slice, verify it runs, commit, update `CURRENT_STATE.md` and this file, then proceed. One sub-phase at a time (build rule #1/#4). **Do not start M8 until the owner confirms M7 and says go.** See `docs/ROADMAP.md` (RIDES MVP track) and `RETURN_CHECKLIST.md`.
