...
- **Current sub-phase:** **M9 — Customer intelligence. ✅ COMPLETE — inactive-customer detection (pure + unit-tested), dashboard follow-up panel, copy-only drafts, and the `get_inactive_customers` Ask tool (2026-07-21).**
- **Next sub-phase:** **M10 — Customer-Zero field test (30-day real usage + `CUSTOMER_ZERO_FEEDBACK.md`). NOT a build phase — the RIDES MVP build (M1–M9) is complete.**
...
**The RIDES MVP build (M1–M9) is complete.** M9 (customer intelligence) is done; the deferred Ask tool is wired in.

**M10 — Customer-Zero field test (NOT a build phase):**

> The owner runs OBSIDIAN on Midnight Rydes for ~30 days: logging trips/expenses (form + natural language), trusting the dashboard numbers, asking OBSIDIAN, and using follow-up drafts. Capture findings in `CUSTOMER_ZERO_FEEDBACK.md`. There is no feature to build here — only real usage, observation, and small fixes as issues surface.

**Before onboarding any second real user/business:** run the deferred **negative second-user tenant-isolation test** (the HARD GATE above). Post-MVP feature ideas (voice, auto-SMS, Towing/Beauty verticals, etc.) remain excluded until explicitly approved. See `docs/ROADMAP.md` and `RETURN_CHECKLIST.md`.
