...
- **Current sub-phase:** **M8 — Natural-language trip entry. ✅ BUILT & statically verified (tsc + M5 tests + next build + boot + client-bundle leak check, 2026-07-21); owner reviews the prefilled form (the built-in confirm step) with a key set.**
- **Next sub-phase:** **M9 — Customer intelligence. NOT STARTED. Do not begin until the owner explicitly says go.**
...
- **M9 onward:** customer intelligence, field test. None started.
...
M8 is built (natural-language trip entry, parse-only + M4 review/save); owner reviews on screen. The deferred `getInactiveCustomers` Ask tool belongs in M9.

**M9 (Customer intelligence):**

> Add customer-intelligence detection over the M3 data in `src/lib/business` (pure, tested): inactive-customer detection (repeat customers with no completed trip in N days — e.g. 90), repeat-customer / lifetime-value insight, and follow-up message **drafts only — no auto-send** (SMS/auto-send is an excluded feature; drafting is Level-2 "prepare"). Surface on the dashboard's Customer Insights (and optionally add the `get_inactive_customers` Ask tool). "Inactive" needs a reference date — reuse the M5 date helpers / pass an explicit "as of" date so the detection stays pure and testable. Keep to this sub-phase only; stop and verify before M10.

Do the smallest runnable slice, verify it runs, commit, update `CURRENT_STATE.md` and this file, then proceed. One sub-phase at a time (build rule #1/#4). **Do not start M9 until the owner confirms M8 and says go.** See `docs/ROADMAP.md` (RIDES MVP track) and `RETURN_CHECKLIST.md`.
