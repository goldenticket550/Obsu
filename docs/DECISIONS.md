...
**Status:** Accepted. Requires `ELEVENLABS_API_KEY` in `apps/web/.env.local` (server-only, git-ignored, never `NEXT_PUBLIC`). The voice id / STT+TTS model ids are single config constants in `src/lib/voice/elevenlabs.ts`. Remaining queued follow-up: voice-driven trip logging (speech → M8 parse). Next step after this: deploy to Vercel (set `ELEVENLABS_API_KEY` + the existing env in the Vercel project).
