...
**Status:** Accepted. The MVP exclusion "voice/ElevenLabs" is now satisfied **only** for browser-native voice I/O. Still excluded pending explicit approval: **ElevenLabs** premium voice (planned via the swappable TTS layer), **auto-SMS/any message sending**, and the other excluded features. Queued follow-ups: (a) ElevenLabs TTS implementation, (b) voice-driven trip logging (speech → the M8 parse flow). **Superseded in part by ADR-013** (browser SpeechRecognition replaced by server-side STT; ElevenLabs voice added).

### ADR-013 — Real voice: server-side STT + ElevenLabs TTS (supersedes M11's browser SpeechRecognition)
**Decision:** Replace the browser Web Speech API input path (ADR-012) with **server-side speech-to-text**, and implement the swappable TTS with an **ElevenLabs** backend. Provider: **ElevenLabs for both** STT (**Scribe**, `scribe_v1`, `POST /v1/speech-to-text`) and TTS (`eleven_multilingual_v2`, `POST /v1/text-to-speech/{voice_id}`, default voice "Sarah" `EXAVITQu4vr4xnSDxMaL`) — one `ELEVENLABS_API_KEY` covers both. The browser records audio (getUserMedia + MediaRecorder, tap-to-toggle + silence auto-stop, AnalyserNode for the orb) and POSTs it to new **server-only** routes `/api/voice/transcribe` and `/api/voice/speak`; all provider calls and the key stay server-side. TTS audio is decoded in the browser via Web Audio so the orb pulses to the real voice; **browser SpeechSynthesis remains the automatic fallback** if the key/route fails. **Still no change to the answer path** — the transcript goes through the same M7 `askAction`/tools (no fabrication, no new answer logic).
**Why:** Browser SpeechRecognition proved unreliable — it depends on the browser's cloud speech service and threw `network` on the owner's Edge/Surface setup, and doesn't work consistently across browsers/phones. Server-side STT works on any browser that can record audio (incl. mobile) and keeps the provider key off the client. Choosing ElevenLabs for STT **and** TTS means a single key/vendor for the whole voice pipeline (the alternative — OpenAI Whisper for STT — would have required a second key/provider; deliberately avoided).
**Status:** Accepted. Requires `ELEVENLABS_API_KEY` in `apps/web/.env.local` (server-only, git-ignored, never `NEXT_PUBLIC`). The voice id / STT+TTS model ids are single config constants in `src/lib/voice/elevenlabs.ts`. **Free-tier constraint (verified 2026-07-21 against the owner's key):** the owner's ElevenLabs plan is free, and the API returns 402 `paid_plan_required` for **Voice Library** voices (incl. "Rachel") — so the default is a **default** voice ("Sarah"; Brian/Daniel also confirmed usable). TTS + STT (Scribe) both work on the free plan with a default voice (validated end-to-end: TTS→mp3→Scribe round-tripped correctly). On a paid plan any voice id works — swap the one constant. Remaining queued follow-up: voice-driven trip logging (speech → M8 parse). Next step after this: deploy to Vercel (set `ELEVENLABS_API_KEY` + the existing env in the Vercel project).
