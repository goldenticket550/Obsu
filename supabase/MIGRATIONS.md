...
| `0011_core_exclude_fixture_orgs.sql` | Additive, read-only replacement of the four Core RPCs to exclude cross-tenant proof fixture organizations | applied | 2026-08-07 | Applied cleanly through the production SQL editor with no schema or table changes. Authenticated owner execution of `core_portfolio_summary()` returned exactly Midnight Rydes, Covered by CCG, and Infinite Beauty Palace, with no `ZZ-RLS-TEST-*` rows. Apply to staging when the staging project exists. |
