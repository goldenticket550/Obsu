...
export function RouteLine({ pickup, dropoff, editHref, variant = "card" }: {
  pickup?: string | null;
  dropoff?: string | null;
  editHref: string;
  variant?: "card" | "scene";
  ...
  const missingLabel = state === "empty" ? "Add route details" : state === "pickup-only" ? "Add destination" : "Add pickup";
  const ribbonStateClass = state === "pickup-only" || state === "destination-only" ? styles[state] : "";
  return (
    <div className={`${styles.route} ${variant === "scene" ? styles.sceneRoute : ""}`} data-route-state={state}>
      <p className="sr-only">{`Route: from ${from} to ${to}. Schematic only, not a map.`}</p>
      <svg className={`${styles.ribbon} ${ribbonStateClass}`} viewBox="0 0 520 72" preserveAspectRatio="none" aria-hidden="true" focusable="false" role="presentation">
